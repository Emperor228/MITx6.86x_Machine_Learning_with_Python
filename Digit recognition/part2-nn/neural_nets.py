...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        output_layer_error = output_layer_activation_derivative(output)*(activated_output-y)
        hidden_layer_error = np.asmatrix(np.asarray(vec_ReLU_derivative(hidden_layer_weighted_input))*np.asarray(self.hidden_to_output_weights.T)*output_layer_error[0,0])
        bias_gradients = np.asmatrix(np.array([[1],[1],[1]])*np.asarray(hidden_layer_error))

        hidden_to_output_weight_gradients = output_layer_error[0,0]*hidden_layer_activation
        input_to_hidden_weight_gradients = np.matrix(np.array([[x1,x2],[x1,x2],[x1,x2]])*np.asarray(hidden_layer_error))
    ...
    def predict(self, x1, x2):
        ...
        output = self.hidden_to_output_weights * hidden_layer_activation
        activated_output = output_layer_activation(output)
        return activated_output[0].item()
